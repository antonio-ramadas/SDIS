...
public class SpaceReclaiming implements Connection {
    ...
    /**
     * Melhoramento
     * Se o grau de repliação for menor que o desejado e o peer não iniciar o protocolo de backup
     * espera um tempo aleatório de 30 a 60 segundos, verifica o grau de replicação
     * e se for menor inicia o protocolo de backup
     */
    ...
    public void handleReceived() {
        ...
        if (Backup.getInstance().isStored(chunk)) {
            ...
            if (!chunk.canBeDeleted()) {
                ...
                sleep(WAIT_TIME + ((int) (Math.random()*100000))%WAIT_TIME);
